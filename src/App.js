...
let [minutes, setMinutes] = useState(undefined)
let [seconds, setSeconds] = useState(undefined)
...
let addTask = (e) => {
  if (checkSpace(e.target.value)) setValue(e.target.value)   
  return
  }

//установка минут таймера
let addMinutes = (e) => {
  if (checkSpace(e.target.value)) setMinutes(e.target.value)
  return
};
...
let addSeconds = (e) => {
  ...
  if (e.code === 'Enter' && checkSpace(e.target.value) && checkSpace(minutes) && checkSpace(seconds) && value) {
  tasks.push(task)
...
//   // изменения свойства в стейте для того,
  ...
  let taskEdit = (index) => {
    ...
    console.log(tasks[index].editing)
      const oldItem = tasks[index];
...
let editMinutesState = (e, index) => {
  ...
  setTask(currentTask.minutes = e.target.value)
}
...
let editSecondsState = (e, index) => {
  const {editing} = tasks[index]
  const oldItem = tasks[index]
  const newItem = {...oldItem, seconds: oldItem.seconds = e.target.value}
  if (e.code === 'Enter' && value && minutes && seconds && editing) {

      const newArr = [...tasks.slice(0, index), {...newItem, editing: !newItem.editing}, ...tasks.slice(index + 1)]
    console.log(newItem)
      setTasks(newArr)
  }
};
